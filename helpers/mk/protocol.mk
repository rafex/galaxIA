...
# distribuibles (@rafex/galaxia-fhs-protocol, @galaxia/atlas,
# @galaxia/navigator, @galaxia/portal-chat) a GitHub Packages.
...
atlas-bump-check:
	$(call section,Verificando si hace falta subir la versión de @galaxia/atlas)
	@GH_TOKEN=$${GH_TOKEN:?"GH_TOKEN requerido — export GH_TOKEN=\$$(gh auth token)"} \
		uv run helpers/python/bump_package_version.py apps/atlas --check

atlas-bump:
	$(call section,Subiendo versión de @galaxia/atlas si ya está publicada)
	@GH_TOKEN=$${GH_TOKEN:?"GH_TOKEN requerido — export GH_TOKEN=\$$(gh auth token)"} \
...
atlas-publish: atlas-bump atlas-verify
	$(call section,Publicando @galaxia/atlas a GitHub Packages)
	npm publish -w apps/atlas
	$(call ok,Publicado)

.PHONY: navigator-bump-check navigator-bump navigator-verify navigator-publish
navigator-bump-check:
	$(call section,Verificando si hace falta subir la versión de @galaxia/navigator)
	@GH_TOKEN=$${GH_TOKEN:?"GH_TOKEN requerido — export GH_TOKEN=\$$(gh auth token)"} \
		uv run helpers/python/bump_package_version.py apps/navigator --check

navigator-bump:
	$(call section,Subiendo versión de @galaxia/navigator si ya está publicada)
	@GH_TOKEN=$${GH_TOKEN:?"GH_TOKEN requerido — export GH_TOKEN=\$$(gh auth token)"} \
...
navigator-publish: navigator-bump navigator-verify
	$(call section,Publicando @galaxia/navigator a GitHub Packages)
...
portal-chat-bump-check:
	$(call section,Verificando si hace falta subir la versión de @galaxia/portal-chat)
	@GH_TOKEN=$${GH_TOKEN:?"GH_TOKEN requerido — export GH_TOKEN=\$$(gh auth token)"} \
		uv run helpers/python/bump_package_version.py apps/portal-chat --check

portal-chat-bump:
	$(call section,Subiendo versión de @galaxia/portal-chat si ya está publicada)
	@GH_TOKEN=$${GH_TOKEN:?"GH_TOKEN requerido — export GH_TOKEN=\$$(gh auth token)"} \
...
portal-chat-publish: portal-chat-bump portal-chat-verify
	$(call section,Publicando @galaxia/portal-chat a GitHub Packages)
