# SPEC-SATRATING-0001 — Mosquito obligatorio en satélites + fiabilidad del Registry
...
## Vocabulario: "satélite"

A partir de esta spec, en documentación y en la interfaz de usuario llamamos
**satélite** a lo que el protocolo FHS sigue llamando `provider` a nivel de
mensaje (`providerId`, `provider.type`, etc. — **sin cambios de campo**,
para no romper compatibilidad con `examples/llm-provider` y
`examples/ocr-provider` tal como existen hoy). "Satélite" es el nombre que
usamos para hablar de un nodo de la red — coherente con "galaxIA" — pero el
JSON del protocolo no cambia. Cualquier documento nuevo debe usar
"satélite" al describir el concepto para la audiencia; el código y el
...
1. **El "mosquito" (dispatcher concurrente) está documentado pero no
   ...
   - No hay una señal explícita de que una petición (`chat.request` /
     `tool.call`) **ya fue tomada por el dispatcher** del satélite. Hoy
     ...
     satélite tarda, no hay forma de distinguir "está formando la cola y
...
2. **El Registry no sabe qué tan rápido o confiable es cada satélite.**
...
- **Protocolo**: nuevo mensaje `dispatch.ack` que el satélite envía
...
- **Registry con historial de métricas por satélite**: un store en memoria
...
- **Solo lectura**: el rating se **expone**, no se **usa** todavía para
  decidir qué satélite resolver ante varios candidatos — eso es la
...
- **Renombrar campos del protocolo o del código** (`provider`, `Provider*`,
  `providerId`) a "satélite"/`satellite`. Es un cambio de vocabulario en
  documentación/UI únicamente en esta iteración — ver "Vocabulario" arriba.
...
puede medir "cuánto tarda este satélite en *empezar* a trabajar" por
separado de "cuánto tarda en *terminar*" — y esa distinción importa: un
satélite con cola llena (mosquito saturado) puede tardar mucho en aceptar
...
sequenceDiagram
    ...
    participant SAT as Satélite (LLM u OCR)
...
- Obligatorio para **todo** `chat.request` y `tool.call` que el satélite
  acepte — enviarlo es parte del contrato de `docs/protocolo-provider.md`,
  no opcional.
- Si el satélite rechaza la petición de inmediato (`UNSUPPORTED_CAPABILITY`,
...
### Qué pasa si el satélite nunca manda `dispatch.ack`

Un satélite construido antes de esta spec (o que no implementa el
...
| Un satélite mal implementado envía `dispatch.ack` con `requestId` que no corresponde | Medio | Mismo tratamiento que cualquier mensaje con `requestId` inválido hoy — se ignora si no hay una petición pendiente con ese id (ya es el comportamiento de `sendAndWait` en `mcp-host.ts`) |
...
- [ ] El Registry acumula muestras de latencia de despacho/total/éxito por
      satélite + capability, con ventana acotada (no crece sin límite).
...
- [ ] Un satélite que no envía `dispatch.ack` (comportamiento anterior a
...
- `spec-native/specs/rag-provider/SPEC.md` — otra iniciativa en draft que
  también depende del comportamiento de providers/satélites; sin relación
