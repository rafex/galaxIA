...
| satelite-rating | draft | TASK-SATRATING-0001..0008 | DEC-0009 (dispatcher concurrente, formalizado ahora), DEC-0024 (vocabulario) | `packages/fhs-protocol/src/`, `apps/agent-server/src/registry/`, `apps/agent-server/src/agent/runtime.ts`, `apps/agent-server/src/providers/`, `examples/llm-provider/`, `examples/ocr-provider/` | — | Solo documentado, sin implementar. Formaliza el "mosquito" (DEC-0009, `docs/protocolo-provider.md`) con un mensaje `dispatch.ack` verificable, y agrega historial de latencia + rating por nodo en el Registry. Usa "nodo" como término neutro (DEC-0024) — "estrella"/"satélite" solo cuando la distinción LLM/tool importa — sin cambios de campo en el protocolo. |
...
| vocabulario-espacial | in_progress | TASK-VOCAB-0001..0005 | DEC-0024 | `spec-native/specs/satelite-rating/SPEC.md`, `spec-native/specs/p2p-discovery/SPEC.md` (ya alineadas), `docs/vocabulario.md` (pendiente), `site/index.md` (pendiente) | Decisión tomada 2026-07-05 (Opción B): Star=LLM, Satellite=tools, Node=neutro. `satelite-rating`/`p2p-discovery` y sus `TASKS.md` actualizados a este vocabulario | Falta el documento canónico (TASK-VOCAB-0002) y la frase de posicionamiento en el portal (TASK-VOCAB-0004) — ninguno bloqueante para seguir con código. La reformulación de FHS (TASK-VOCAB-0004b) y el renombrado de código/contenedores (fuera de alcance) se resolvieron/registraron aparte. |
