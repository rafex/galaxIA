...
- Close criteria: verificado 2026-07-05 con un stack local real (agent-server + `examples/llm-provider` real contra un LLM mock HTTP + `examples/ocr-provider` real contra un ether-ocr mock HTTP, ambos providers como procesos reales hablando FHS WebSocket real, no simulado):
  ...
  - OCR con adjunto real: `tool.selected` → `tool.running` → `tool.completed` → `ocr.extracted` con el texto del mock; el contexto se reutiliza automáticamente, sin `attachment.decision`. `GET /api/fhs/providers` mostró `{ capability: "document.ocr", rating: 5, avgDispatchMs: 0, avgTotalMs: 17, successRate: 1, sampleCount: 1 }`.
