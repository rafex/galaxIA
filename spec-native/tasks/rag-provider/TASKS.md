...
- Close criteria histórico: en el punto donde `chat-ws.ts` resolvía `attachment.decision { use: true }`, se llamaba a `document_index` (vía `indexForRag()`). Ese mecanismo fue superseded: la integración vigente debe indexar después del OCR automático y del `DocumentContext`; no existe confirmación manual. En mensajes siguientes de una conversación marcada, `runtime.run(..., ragActive: true)` llama a `document_query` antes de la llamada al LLM y antepone los chunks sin exponerlos en la UI (`queryRagContext`, silencioso, sin eventos `tool.*`).
