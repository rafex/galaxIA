...
- **Iniciativa en spec, sin iniciar:** `satelite-rating` — mensaje `dispatch.ack` obligatorio (formaliza el "mosquito"/dispatcher concurrente de DEC-0009 con una señal verificable en el protocolo) + historial de latencia y rating por satélite en el Registry (SPEC-SATRATING-0001 en `spec-native/specs/satelite-rating/SPEC.md`). Adopta "satélite" como vocabulario de producto para lo que el protocolo sigue llamando `provider`. No se inicia hasta decisión explícita.
- **Iniciativa en spec, bloqueada en una decisión:** `vocabulario-espacial` — tabla de vocabulario coherente con la marca "GalaxIA" (Atlas=Registry, Portal=chat web, Navigator=agente/orquestador, Beacon=manifiesto, Pulse=heartbeat, Flight Log=procedencia, y más) (SPEC-VOCAB-0001 en `spec-native/specs/vocabulario-espacial/SPEC.md`). **Bloqueada** en decidir si "satélite" sigue siendo el término general para cualquier nodo (ya usado en `satelite-rating`/`p2p-discovery`) o si se reserva "Star" para LLM y "Satellite" solo para tools — conflicto directo entre lo ya decidido y la propuesta nueva. No toca código, contenedores ni el protocolo FHS en ningún escenario.
