...
- **Iniciativa en spec, sin iniciar:** `satelite-rating` — mensaje `dispatch.ack` obligatorio (formaliza el "mosquito"/dispatcher concurrente de DEC-0009 con una señal verificable en el protocolo) + historial de latencia y rating por nodo en el Registry (SPEC-SATRATING-0001 en `spec-native/specs/satelite-rating/SPEC.md`). Usa "nodo" como vocabulario de producto genérico ("estrella" para LLM, "satélite" para tools, DEC-0024) para lo que el protocolo sigue llamando `provider`. No se inicia hasta decisión explícita de empezar a codificar.
