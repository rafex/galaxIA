...
- **Implementado, pendiente de aplicar en el despliegue real:** TLS/WSS con certificado autofirmado para todo el protocolo FHS (DEC-0023, `docs/tls-autofirmado.md`). Opt-in vía overlay `containers/compose.tls.yaml` — no afecta el despliegue sin TLS. Verificado en local (agent-server + ambos providers, arranque y registro por `wss://`); falta aplicarlo sobre la topología multi-host real y repetir el checklist end-to-end sobre el canal cifrado.
