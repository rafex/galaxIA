...
- **Consecuencias:**
  ...
  - Pendiente no bloqueante: exponer `maxWaitMs` como control visible en la barra de configuración del `Portal`; no depende del flujo OCR, cuyo antiguo `ocrMode` fue retirado por la actualización de contrato del 2026-08-05.
...
> **Superseded 2026-08-05:** el modo `confirm` y la selección configurable se
> eliminaron del contrato. OCR es siempre automático; la vista previa se
> conserva y el texto reutilizable viaja como `DocumentContext` Protobuf.
...
- **Decisión — ambos providers implementados con un motor/proceso interno deliberadamente mínimo, documentado como no-recomendación:**
  ...
  - **Orquestación en Navigator (no en los providers):** RAG reutiliza el resultado de OCR automático y el `DocumentContext` estructurado; `indexDocumentForRag()`/`queryRagContext()` se disparan después de resolver ese contexto, sin `attachment.decision`. KB conserva su flujo propio `kb.recommended`/`kb.decision` para el modo recomendado; el modo manual (`preferences.kb`) resuelve directo sin confirmación.
...
## DEC-0070 — El publish a npmjs.org debe reescribir `publishConfig.registry` (nunca ha llegado un paquete a npmjs)

- **Fecha:** 2026-07-10
- **Estado:** `accepted` — implementado
- **Contexto:** al investigar el fallo del primer publish real tras el rename (run 29100658504, merge de DEC-0068), se encontró que el segundo `npm publish --registry https://registry.npmjs.org` en realidad publicó **de nuevo a npm.pkg.github.com** (el log lo muestra: "Publishing to https://npm.pkg.github.com/ with tag latest and public access") y falló con `E409 Cannot publish over existing version`. Causa: en el npm 10 del runner (Node 20), `publishConfig.registry` del package.json tiene prioridad sobre el flag `--registry` de la CLI (en npm 11 local el flag gana — el comportamiento cambió entre versiones, por eso no se detectó en verificación local). Además se descubrió que el run "exitoso" de DEC-0067 en realidad **saltó el paso de publicación** (no detectó workspaces cambiados) — es decir, **ningún paquete ha llegado jamás a npmjs.org**; la fase 6 (DEC-0066) nunca se había ejercitado de verdad.
- **Decisión:** antes del segundo publish, el workflow reescribe `publishConfig.registry` a npmjs con `npm pkg set ... -w "$pkg"` y lo restaura después con `git checkout -- "$pkg/package.json"`. Se mantiene el flag `--registry` por claridad (y porque en npm 11 sí funciona).
- **Consecuencias:** `.github/workflows/publish-packages.yml`. El estado divergente que dejó el fallo (`@rafex/galaxia-atlas@0.1.0` solo en GitHub Packages) se autocorrige en el siguiente run: el bump detecta 0.1.0 publicada y sube a 0.1.1 para ambos registros. Verificado con `npm publish --dry-run` en ambos modos.
- **Addendum (2026-07-11, run 29138370429):** el fix de `npm pkg set` solo **tampoco bastó** — en npm 10, `npm publish -w` ignora incluso el `publishConfig` reescrito del workspace y volvió a publicar a GitHub Packages (E409 en fhs-protocol, que sí alcanzó a subir 0.1.12 a GH). Fix definitivo: publicar parado **dentro del directorio del paquete** (sin `-w`), con publishConfig reescrito + flag `--registry`, y verificar en el log de npm ("Publishing to https://registry.npmjs.org") que el publish fue al registro correcto — falla rápido y claro si npm cambia de comportamiento otra vez.
- **Addendum 2 (2026-07-11, run 29138559277):** el guard funcionó y reveló la causa real de fondo: `npm pkg set publishConfig.registry=... -w <pkg>` es un **no-op silencioso en npm 10** — el publishConfig nunca cambió, y como en npm 10 ese campo le gana al flag `--registry`, todos los intentos iban a GH. Fix final: editar el package.json con `jq` (no puede fallar en silencio) + log del publishConfig efectivo antes de publicar. El guard del registro se queda como red permanente.
- **Addendum 3 (2026-07-11, run 29138772605):** con publishConfig ya en npmjs el publish SIGUIÓ yendo a GH — la config que manda es la **por scope**: `actions/setup-node` con `registry-url` de GitHub Packages escribe `@rafex:registry=https://npm.pkg.github.com` en su `.npmrc` temporal (scope autoderivado del owner del repo), y en npm la config de scope le gana al `--registry` genérico y al `publishConfig.registry`. Además, setup-node apunta `NPM_CONFIG_USERCONFIG` a ese archivo temporal — el token de npmjs escrito en `~/.npmrc` era invisible para npm. Fix: token a `${NPM_CONFIG_USERCONFIG:-~/.npmrc}` y sobreescribir el registro del scope en las tres capas (`publishConfig` con jq incluyendo `@rafex:registry`, y flag CLI `--@rafex:registry=`).
- **Addendum 4 (2026-07-11, run 29138983772):** el registro por fin apuntó a npmjs pero npm respondió `ENEEDAUTH` aunque el token estaba en el archivo de `NPM_CONFIG_USERCONFIG`. Solución final: dejar de compartir el `.npmrc` de setup-node — el publish a npmjs corre con un **userconfig limpio propio** (`NPM_CONFIG_USERCONFIG=/tmp/npmjs-userconfig`, solo el token de npmjs), eliminando todas las capas de interferencia de una vez.

## DEC-0071 — En npmjs.org los paquetes se publican bajo `@rafex_labs/` (el scope `@rafex` no es del proyecto)
...
## DEC-0073 — Regresión funcional E2E real en CI (SPEC-QUALITY-0001, pieza 2)

- **Fecha:** 2026-07-11
- **Estado:** `accepted` — implementado
- **Contexto:** SPEC-QUALITY-0001 quedó `proposed` desde 2026-07-06 señalando tres huecos; al retomarla se confirmó que las piezas 1 (tests) y 3 (lint/formato) ya se habían resuelto en trabajo previo (ESLint real, Vitest en los 4 workspaces) — solo faltaba la pieza 2: un workflow que ejerza el sistema real, no solo "compila".
- **Decisión:** `scripts/e2e-smoke.ts` automatiza el patrón de verificación manual del proyecto — levanta Atlas + Navigator reales, registra un Star mock (identidad Ed25519 real, payloads endurecidos de DEC-0069) y ejerce un chat completo por la API pública, verificando en el camino: firma del `welcome`, firma de `register` (hash de manifiesto), `visibility` propagada en el catálogo REST, y `CallerAuth` en el `chat.request` entrante. Se integró como job `e2e-smoke` en `ci.yml`, corriendo en paralelo a lint/typecheck/test/build.
- **Hallazgo real en la primera corrida:** `AgentRuntime.callLlm` crasheaba sin capturar (`response.toolCalls.length` sobre `undefined`) si un provider respondía `chat.completed` sin `toolCalls` — la conversación quedaba colgada sin ningún error visible. Corregido normalizando `toolCalls ?? []` en `llm-gateway.ts` (ambos paths, generate y stream) al recibir el mensaje del wire — exactamente la clase de bug que typecheck no detecta porque el tipo TS declara el campo obligatorio, pero nada lo garantiza en el JSON real de otro proceso.
- **Consecuencias:** `scripts/e2e-smoke.ts` (nuevo), `.github/workflows/ci.yml` (job `e2e-smoke`), `apps/navigator/src/providers/llm-gateway.ts`, `spec-native/specs/quality-gates/SPEC.md` (estado `accepted`). Verificado localmente contra un Atlas+Navigator reales antes de subir a CI.

## DEC-0074 — Puente de eventos Atlas↔Navigator vía NATS, opt-in (SPEC-BRIDGE-0001)

- **Fecha:** 2026-07-11
- **Estado:** `accepted` — implementado
- **Contexto:** SPEC-BRIDGE-0001 (`proposed` desde DEC-0035) documentaba que `node.online`/`node.lost` quedan atrapados en el `EventBus` interno de Atlas sin ningún consumidor real desde que Atlas y Navigator son procesos separados — el Portal no se entera en vivo de altas/bajas de nodos, solo por consulta REST puntual.
- **Decisión:** NATS core (pub/sub simple, sin JetStream), **opt-in** vía `NATS_URL` en ambos procesos (mismo patrón que `MDNS_ENABLED`/`TLS_CERT_PATH`, DEC-0032). Atlas publica a `fhs.node.online`/`fhs.node.lost` suscribiéndose a su propio `EventBus.subscribeToRuntime()` (cero cambios en `registry.ts`, DEC-0074-approach: el bridge es un handler más). Navigator se suscribe a esos subjects y reinyecta cada evento en su propio `EventBus` — mismo canal que ya sirve conversaciones al Portal, sin una segunda conexión directa a Atlas. Degradación graceful en ambos lados: si `NATS_URL` está seteado pero la conexión falla, cae a no-op con un warning — nunca tumba el proceso.
- **Sin snapshot inicial ni persistencia:** innecesario porque `AgentRuntime` resuelve el catálogo por REST en cada turno; esto es puramente una mejora de experiencia (notificaciones en vivo), nunca una dependencia funcional del chat.
- **Consecuencias:** `apps/atlas/src/atlas/nats-bridge.ts` (nuevo), `apps/navigator/src/nats-bridge.ts` (nuevo), `apps/{atlas,navigator}/src/index.ts` (wiring), `containers/compose.nats.yaml` (nuevo overlay opt-in), dependencia exacta `nats@2.29.3` en ambos workspaces, tests (`__tests__/nats-bridge.test.ts` en ambos). `spec-native/specs/atlas-navigator-bridge/SPEC.md` actualizado a `accepted`.
- **Bug real encontrado al verificar de punta a punta:** con `NATS_URL` seteado pero el servidor caído/inalcanzable, ambos procesos loggeaban `"Puente NATS activo..."` de todas formas — el log dependía solo de que la variable estuviera presente, no de que la conexión hubiera realmente ocurrido (que sí degradaba correctamente a no-op, solo el mensaje mentía). Corregido: `NatsBridge`/`NatsBridgeConsumer` exponen `connected: boolean`, y `index.ts` en ambos procesos solo loggea "activo" cuando es `true`.

## DEC-0075 — Verificación real contra hardware físico (bastion-wifi): fixes de Justfile y podman-compose

- **Fecha:** 2026-07-11
- **Estado:** `accepted` — implementado
- **Contexto:** con acceso recuperado al bastion real (issue #1, antes bloqueado por falta de hardware), se ejecutó por primera vez una verificación de punta a punta contra máquinas físicas reales del stack endurecido de esta sesión (DEC-0069/0072/0073/0074): `llama-server` real (`qwen2.5-coder-3b-instruct`, CPU-only) + Atlas/Navigator/Portal-chat (repo `galaxIA`, build fresco) + un `star` **sin migrar** de `galaxIA-satellite-star` (payload legado, exactamente el caso que DEC-0069 debía seguir aceptando).
- **Resultado de la verificación (todo real, sin mocks):**
  - `star` legado se registró correctamente contra el Atlas endurecido — confirma en producción que el fix crítico de timestamps (ms) no rompió la compatibilidad hacia atrás, y que el fallback de firma legada de `register` sigue funcionando.
  - `visibility: "community"` propagada correctamente en `/api/fhs/providers` (feature de DEC-0069).
  - Chat E2E real completo: `chat.request` → `dispatch.ack` → inferencia real (~22.5s en CPU) → `chat.completed` → SSE `assistant.completed` en el Portal.
  - **`satelite-rating` verificado contra hardware real por primera vez** (issue #1, cierra el pendiente "repetir contra los 3 equipos reales"): fila real en `atlas-metrics.db` con `dispatch_ms=18`, `total_ms=22560`, `success=1`.
- **Dos bugs reales encontrados y corregidos en el proceso:**
  1. `Justfile` (`container-up-core`/`container-up-core-tls`) listaba el servicio de compose como `portal`, nombre obsoleto desde el rename a `portal-chat` — nunca se había ejecutado ese recipe contra un compose real desde el rename. Corregido.
...
## DEC-0078 — Cerrar los 4 hallazgos de CodeQL: TLS pineable + timeout acotado en clientes WebSocket

- **Fecha:** 2026-07-11
- **Estado:** `accepted` — implementado (lado `apps/navigator`; `galaxIA-satellite-star` queda pendiente, ver más abajo)
- **Contexto:** CodeQL marcó 4 alertas `high` preexistentes en `main` (ya presentes antes de esta sesión, expuestas al abrir PR #64): `js/disabling-certificate-validation` en `llm-gateway.ts:61`/`mcp-host.ts:76` (el `rejectUnauthorized: false` incondicional para `wss://`, documentado como trade-off aceptado de PoC en `docs/tls-autofirmado.md` pero nunca resuelto), y `js/resource-exhaustion` en `llm-gateway.ts:142`/`mcp-host.ts:274` (`setTimeout` con duración controlada por el cliente vía `preferences.maxWaitMs`, sin acotar).
- **Decisión:**
  - **TLS**: nuevo módulo compartido `apps/navigator/src/providers/ws-security.ts`. Default ahora **seguro** (verificación TLS estándar de Node) en vez de `rejectUnauthorized: false` incondicional. `TLS_CA_CERT_PATH` (nuevo, recomendado) fija el certificado autofirmado como única CA confiable — implementa la recomendación que `docs/tls-autofirmado.md` ya documentaba ("fijar el certificado esperado, no desactivar la verificación") pero nunca tuvo código detrás. `FHS_TLS_INSECURE=true` (nuevo, opt-in explícito) preserva el comportamiento anterior para quien no pueda distribuir el cert, con un warning en el log.
  - **Timeout**: `clampTimeoutMs(timeoutMs, fallback)` acota `preferences.maxWaitMs` a `[1_000, 600_000]` ms antes de usarlo en cualquier `setTimeout` — protege contra valores no finitos (`NaN`/`Infinity`), negativos/cero (dispararían el timer de inmediato) y valores absurdamente grandes (mantener recursos vivos indefinidamente), sin reducir el rango útil real (el default ya documentado es de minutos, no segundos, DEC-0010).
- **Explícitamente no resuelto en esta iteración:** `examples/star-example`/`examples/satellite-ocr-example` en `galaxIA-satellite-star` siguen con `rejectUnauthorized: false` incondicional en su cliente de Registry — mismo patrón, repo distinto, requiere su propio cambio.
- **Consecuencias:** `apps/navigator/src/providers/ws-security.ts` (nuevo), `apps/navigator/src/providers/{llm-gateway.ts,mcp-host.ts}` (usan el módulo compartido), test nuevo (`__tests__/ws-security.test.ts`, 8 casos), `docs/tls-autofirmado.md`. Verificado: typecheck/lint/tests verdes.
- **Limitación de CodeQL encontrada al verificar el PR:** el analizador no reconoce `clampTimeoutMs` como sanitizador interprocedural (`js/resource-exhaustion` sigue marcando el `setTimeout` aunque el valor ya esté acotado), ni distingue el `rejectUnauthorized: false` del branch opt-in (`js/disabling-certificate-validation` marca la línea aunque solo se alcance con `FHS_TLS_INSECURE=true` explícito). Ambos son falsos positivos de una mitigación real, no vulnerabilidades sin resolver. Se probaron comentarios `lgtm[<query-id>]` en la línea exacta — no funcionaron con la configuración de "default setup" de CodeQL de este repo (sin workflow propio de análisis). Se descartaron las 3 alertas por la vía soportada: `PATCH /repos/.../code-scanning/alerts/{n}` con `state=dismissed`, `dismissed_reason="false positive"` y comentario justificando la mitigación real. Los comentarios en el código se dejaron igual, como documentación para quien lea el diff.

## DEC-0079 — TLS pinning seguro en todos los providers de galaxIA-satellite-star

- **Fecha:** 2026-07-30
- **Estado:** `accepted` — implementado (PR #3 en `galaxIA-satellite-star`, pendiente de merge)
- **Contexto:** DEC-0078 cerró el `rejectUnauthorized: false` incondicional en `apps/navigator` pero explícitamente dejó fuera el mismo patrón en `galaxIA-satellite-star` (los 5 ejemplos: `star-example`, `satellite-ocr-example`, `nova-example`, `rag-provider`, `kb-provider`). Todos tenían una función `wsOptions()` local idéntica que retornaba `{ rejectUnauthorized: false }` para cualquier URL `wss://`, independientemente de si había una forma de verificar el certificado disponible.
...
- **Decisión:**
  ...
  2. **`missionId` NO se adopta como campo nuevo del protocolo JSON.** Choca en dos frentes verificados: (a) el rol que cumpliría ya lo cumple `requestId` en `tool.call`/`chat.request` (`packages/fhs-protocol/src/messages.ts`) — sería un campo duplicado para el mismo concepto; (b) violaría la **Fase 3** de DEC-0033 (protocolo JSON en el cable), descartada explícitamente en DEC-0024/DEC-0033/DEC-0034 por el riesgo de flag-day: cambiar un nombre de campo exige actualizar y reiniciar todos los nodos reales desplegados a la vez (hoy Bastion + Raspi4B + Laptop), sin ningún beneficio de protocolo — es puramente cosmético a nivel de wire format.
  3. **Regla general reafirmada** (ya vigente desde DEC-0024, sin cambios): cuando un documento técnico necesita precisión de protocolo, usa el término técnico (`requestId`, `providerId`, `hello`, `register`); cuando habla de cara a un humano (Portal, docs de producto, logs legibles), puede usar el término de `docs/vocabulario.md` (`Mission`, `Beacon`, `Pulse`...) como etiqueta, sin que eso implique un campo nuevo en el JSON.
- **Consecuencias:** ninguna en código — decisión puramente de vocabulario/alcance, no requiere cambios en `packages/fhs-protocol` ni en ningún provider. Sirve como precedente citable la próxima vez que una propuesta externa (o interna) sugiera un término nuevo: primero verificar contra `docs/vocabulario.md` y la Fase 3 de DEC-0033 antes de asumir que hace falta agregar algo.

## DEC-0083 — Logs locales + envío opcional a NATS (`ship` shell + `collect` containerizado), capa ops fuera del protocolo FHS

- **Fecha:** 2026-08-01
- **Estado:** `accepted` — implementado (branch `feat/log-agent-nats`)
- **Contexto:** diagnosticar un incidente real durante esta sesión (un mensaje de chat sin respuesta) requirió SSH a 3 máquinas distintas y revisar `podman logs` uno por uno, sin línea de tiempo unificada ni forma de correlacionar un `conversationId`/`requestId` entre hosts. Se evaluaron 3 niveles de solución: (A) solo actualizar las recetas `just logs-*` existentes a la topología real, (B) un agente local que sigue escribiendo logs en cada host y además los publica a NATS (reusando el bridge opt-in ya existente, `SPEC-BRIDGE-0001`/DEC-0074), (C) un stack de observabilidad nuevo (Loki/Grafana). Se eligió **B**.
- **Decisión explícita de alcance — por qué NO es un cambio de protocolo:** logs son un problema operativo (observabilidad), no un mensaje `hello`/`register`/`chat.request`/etc. Meterlo en `packages/fhs-protocol` habría sido scope creep — `apps/log-agent` es una app de host plano, separada, que nunca toca el wire format del protocolo. Se documenta esta decisión explícitamente para dejar constancia de que la opción de cambiar el protocolo fue considerada y descartada, no simplemente omitida.
- **Diseño — dos piezas separadas, no una sola app:**
  1. **`ship`** (`helpers/scripts/shell/log-agent-ship.sh`, uno por host, corre en los 3): **script de shell, no Node** — sigue `podman logs -f <container>` por cada contenedor configurado. Cada línea se escribe a un archivo local (mismo archivo que ya usaban las recetas `just logs-core`/`logs-llm`/`logs-ocr`, sin cambiarlas) y, si `NATS_URL` está seteada, se publica también a `logs.<host>.<container>` con el CLI de `nats` (binario estático, sin dependencias). **Decisión revisada durante la implementación:** la primera versión de `ship` iba en TypeScript dentro de `apps/log-agent`, pero eso exigía instalar Node en los 3 hosts solo para invocar `podman logs -f` — footprint innecesario (Bastion ni siquiera tenía Node instalado). Un script de shell + el CLI de NATS resuelve lo mismo sin runtime nuevo, más alineado con "hardware modesto" (`spec-native/PRODUCT.md`).
  2. **`collect`** (`apps/log-agent`, TypeScript, **containerizado** vía `containers/log-agent/Containerfile`, corre **una sola vez**, junto al servidor NATS): se suscribe a `logs.>` y arma un archivo agregado `/app/logs/all.log` (montado a un directorio del host con `-v`, formato `[host/container] línea`) — abrible con `lnav -f logs/all.log` para ver los 3 hosts entrelazados por timestamp en una sola vista. A diferencia de `ship`, `collect` no necesita `podman logs -f` de contenedores hermanos (solo habla NATS), así que se containeriza igual que cualquier otro servicio del stack en vez de instalar Node en el host — reusa el cliente oficial de NATS (`apps/log-agent/src/nats-client.ts`, mismo patrón exacto que `apps/atlas/src/atlas/nats-bridge.ts`: opt-in por `NATS_URL`, sin JetStream, falla explícito si no conecta porque sin NATS no hay nada que agregar). **Decisión revisada dos veces durante la implementación:** primero `collect` también se instaló como proceso de host plano en Bastion (Node portable sin sudo) — se revirtió a petición explícita de mantener consistencia con el resto del stack (todo lo que no necesita acceso directo al host corre containerizado).
  3. **Degradación** — `ship` nunca depende de NATS para seguir escribiendo su archivo local (si `nats pub` falla, seguir sin más, mismo patrón que DEC-0074). Sin persistencia/replay en NATS: si `collect` se reinicia, no recupera lo perdido mientras estuvo caído — el archivo local de cada `ship` sigue siendo la fuente completa para ese host.
  4. **Servidor NATS** — se corre vía `podman run` directo (no `docker-compose`/el overlay `containers/compose.nats.yaml` existente), publicando el puerto `4222` a la LAN — el overlay existente solo expone NATS dentro de la red interna del compose, insuficiente para alcanzarlo entre hosts distintos en esta topología.
- **Qué NO se implementa (fuera de alcance, documentado en `docs/observabilidad-logs.md`):** rotación de logs (aceptable para PoC/comunidad en hardware modesto; `logrotate` del SO resuelve esto sin tocar código si se vuelve un problema real), persistencia/replay de NATS (ver punto 3), un cliente NATS persistente en `ship` (el costo de un proceso `nats pub` por línea es aceptable al volumen actual; si crece, la alternativa es volver a un shipper en Node, a cambio de instalar el runtime en ese host).
- **Consecuencias:** `apps/log-agent/` (nuevo, solo `collect`), `containers/log-agent/Containerfile` (nuevo), `helpers/scripts/shell/log-agent-ship.sh` (nuevo), `docs/observabilidad-logs.md` (nuevo, mismo estilo que `docs/despliegue-multi-host.md`). No se tocó `packages/fhs-protocol` ni ningún mensaje del protocolo. Verificado: typecheck/lint/test limpios en el monorepo completo.
...
## DEC-0087 — `FhsMessage` → `Envelope` P2P: routing en el nivel de frame, Handshake 2-step, eliminación de CallerAuth

- **Fecha:** 2026-08-02
- **Estado:** `accepted` — aplicado en IDL (fhs-protocol.proto, asyncapi.yaml)
- **Contexto:** DEC-0086 hizo que el `FhsMessage` proto sea el frame real del wire. Pero el diseño del mensaje tenía redundancias de la época cliente-servidor: (a) `timestamp` repetido en cada mensaje individual, (b) `callerId` repetido en `CallerAuth` aunque el identificador del emisor ya puede estar en el envelope, (c) flujo de registro en 4 pasos (hello → welcome → register → registered) donde solo se necesitan 2 para intercambiar lo que importa. Evaluación del estado ALPHA: sin usuarios externos, el coste de un breaking change es cero; el beneficio de un IDL limpio es permanente.
- **Decisión:**
  1. **`FhsMessage` → `Envelope`.** El tipo raíz se renombra a `Envelope` y añade 6 campos de routing en el nivel superior (fields 1-6), fuera del `oneof payload`: `message_id` (UUID, deduplicación), `source_peer_id` (DID del emisor), `dest_peer_id` (DID del destino, vacío = broadcast), `timestamp` (Unix ms, anti-replay de envelope), `version` ("1"), `signature` (Ed25519 del envelope). Los payloads renumeran su `oneof` a 10-51 para dejar espacio semántico entre grupos.
  2. **Eliminación de `CallerAuth`.** Con `Envelope.source_peer_id` + `Envelope.signature`, el struct `CallerAuth { callerId, signature, timestamp }` es completamente redundante. Se elimina del IDL. TASK-IDL-008 queda resuelto por diseño.
  3. **Flujo de registro 4-step → 2-step.** Se eliminan `HelloMessage`, `WelcomeMessage`, `RegisterMessage`, `RegisteredMessage`. Se añaden `HandshakeMessage` (Provider → Atlas: `fhsVersion`, `listenAddrs`, `beacon`) y `HandshakeAckMessage` (Atlas → Provider: parámetros de lease). La identidad del provider viene de `Envelope.source_peer_id`; la firma del Envelope autentica el handshake de forma bilateral (cada sentido lleva su firma). La FSM de conexión pasa de 4 estados a 2.
  4. **Eliminación de `timestamp` en mensajes de misión.** Los campos `timestamp: int64` en `ChatRequestMessage`, `ChatCancelMessage`, `ChatDeltaMessage`, `ChatCompletedMessage`, `ChatErrorMessage`, `ToolCallRequestMessage`, `ToolCancelMessage`, `ToolCallResultMessage`, `ToolCallErrorMessage`, `ToolListRequestMessage`, `ToolListResponseMessage` se eliminan — el `Envelope.timestamp` cubre anti-replay para todos. Excepciones: `PongMessage.server_timestamp` (para calcular RTT) y `DispatchAckMessage.queued_at` (timestamp de negocio — cuándo entró la misión en la cola, diferente de cuándo se envió el frame).
  5. **Simplificación de `AtlasAnnounceMessage` y `AtlasSyncMessage`.** Los campos `atlas_id`, `timestamp`, `signature` se eliminan — ya están en `Envelope`. `AtlasAnnounceMessage` solo lleva `providerIds`; `AtlasSyncMessage` solo lleva `providers`.
- **Lo que NO cambia:**
  - Los payloads de misión (missionId, messages, toolCalls, etc.) — solo pierden el `timestamp`.
  - El concepto de Beacon, el vocabulario (Star, Satellite, Nova, etc.), los roles de Atlas/Navigator.
  - El LPP framing (DEC-0086) — el `Envelope` es lo que se serializa con LPP.
  - Los campos de negociación de lease y heartbeat — pasan de `WelcomeMessage` a `HandshakeAckMessage`.
- **Por qué ahora y no después:** Estamos en ALPHA sin usuarios externos. El campo `timestamp` en cada mensaje era ruido; `CallerAuth` duplicaba el envelope. Hacer este clean-up después es más costoso (hay código generado en galaxIA-SDK, implementations en galaxIA-Core y galaxIA-satellite-star que ya usen los tipos). Ahora el coste es solo IDL.
- **Consecuencias IDL:** `idl/fhs-protocol.proto` (reescritura completa — ver commit), `idl/asyncapi.yaml` (canal `/register` actualizado, schemas Envelope, Handshake, HandshakeAck; eliminación de CallerAuth y mensajes antiguos de registro). TASKS.md: TASK-IDL-008 marcado `done` por diseño.

## DEC-0088 — Estrategia de implementación: TypeScript valida el IDL; Rust es el destino final

- **Fecha:** 2026-08-02
- **Estado:** `accepted`
- **Contexto:** El IDL FHS P2P (DEC-P2P-001) está definido y publicado como `v0.1.0-p2p-alpha`. La implementación actual de galaxIA-Core (Navigator, Atlas, providers) sigue el modelo centralizado `hello/register/ping` — ningún nodo habla el protocolo P2P todavía. Hay que decidir el lenguaje de implementación, la secuencia, y si se mantiene retrocompatibilidad con el modelo anterior.
- **Contexto técnico:** Los equipos de la red son hardware viejo (Mac mini 2014, Raspberry Pi 4B, laptops de 8-16 GB RAM). Cada MB de RAM y cada ciclo de CPU que consume el runtime de Node.js es un recurso que no está disponible para el modelo LLM o la herramienta. En una Mac mini con 8 GB sirviendo llama.cpp, el runtime TypeScript compite directamente con el modelo por memoria.
- **Decisión:**
  1. **No hay retrocompatibilidad.** El modelo `hello/register/ping/WebSocket centralizado` queda eliminado. galaxIA-Core y galaxIA-satellite-star implementarán directamente el protocolo P2P completo (DHT + GossipSub + stream directo `/fhs/v1/0.1.0`) sin ninguna capa de compatibilidad.
  2. **TypeScript como implementación de validación.** La primera implementación completa del protocolo P2P será en TypeScript/Node.js (galaxIA-Core), usando `libp2p` (js-libp2p). Su único rol es validar que el IDL es correcto e implementable — demostrar que `MissionOffer/Bid/Assign`, `DhtBeaconRecord`, handshake y stream directo funcionan de extremo a extremo. Una vez que pase una prueba end-to-end real (chat + tool call), el IDL se congela.
  3. **Rust es el destino final.** Una vez validado el IDL con TypeScript, los SDKs de producción se implementan en Rust (usando `rust-libp2p`, el más maduro de todos). Rust da binarios de ~5-15 MB sin GC, latencia determinística, y máximo aprovechamiento del hardware modesto. Es el lenguaje prioritario para Stars y Satellites de producción.
  4. **Java/GraalVM Native Image como segunda opción.** Para integraciones con ecosistemas JVM existentes. GraalVM Native Image genera binarios nativos con footprint comparable a Rust pero con acceso al ecosistema Java. Prioridad menor que Rust, misma motivación (reducir overhead en hardware viejo).
