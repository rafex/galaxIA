...
- **laptop** — el core: `apps/portal` (chat), `apps/atlas` (Registry — catálogo de proveedores) y `apps/navigator` (Agent Runtime + Chat API, habla con Atlas por HTTP). Es donde el usuario abre el navegador.
...
|---|---|---|---|
| `apps/portal` (chat) | laptop | `3000` (o el que uses en dev) | El navegador del usuario (localhost en la laptop, o la LAN si otros quieren probar) |
...
```bash
# En la laptop (core): atlas + navigator + portal
just logs-core
