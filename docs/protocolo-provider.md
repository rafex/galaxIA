...
Hoy `examples/star-example` y `examples/satellite-ocr-example` cumplen la mayor parte de este contrato, pero cada uno lo reimplementa desde cero (su propio bucle de conexión, su propio manejo de heartbeat, sus propios códigos de error ad-hoc). Este documento estandariza esas piezas para que el próximo provider — en cualquier lenguaje — no tenga que inventarlas de nuevo.
...
Un manifiesto sin `privacy.retention` **debe ser rechazado por el Registry**, no aceptado con un valor por defecto silencioso — implementado en `apps/atlas/src/atlas/manifest-validation.ts` (DEC-0013), conectado en `ws-handler.ts`: responde `error { code: "INVALID_MANIFEST" }` con el detalle de qué campo falta.
